...
#ifdef ENABLE_PNG
int write_png_image_to_disk(unsigned char* image, const unsigned int WIDTH,
                            const unsigned int HEIGHT, const unsigned int NCOLORS,
                            const char* const filename, const char* const palette);
#endif
int write_ascii_image_to_disk(unsigned char* image, const unsigned int WIDTH,
                            const unsigned int HEIGHT, const unsigned int NCOLORS, const char* const filename);
...
int main(int argc, char **argv) {
    ...
    const double dx = 2*BOUND/(double)(WIDTH-1);
    const double dy = 2*BOUND/(double)(HEIGHT-1);
    ...
    unsigned int num_rows = HEIGHT / size;
    const unsigned int rest = HEIGHT % size;
    ...
    // The first WIDTH uns chars are the first row this process calculated and so on
    // we start from the bottom and work our way upwards.
    unsigned char *color = (unsigned char*) malloc(WIDTH * num_rows * sizeof(unsigned char));
    if (!color) {
    ...
    for (int y = 0; y < num_rows; ++y) {
        ...
        double dimag = local_row_offset * dy - BOUND + VOFFSET;

        // now iterate over all pixels in the row and calculate the color
        for (int x = 0; x < WIDTH; ++x) {
            double dreal = x*dx - BOUND + HOFFSET;
            double _Complex d = dreal + dimag * _Complex_I;
            color[x + y*WIDTH] = calc_mandelbrot_pixel(d, BOUND, NCOLORS);
        }
    }
    ...
    if (rank == 0) {
        ...
        // again the first WIDTH unsigned characters correspond to the
        ...
        int received_rank = 0;
        int received_length = WIDTH*num_rows;
        for (int i = 0; i<size; ++i) {
            ...
            if (i > 0) {
                ...
                // note also that it is legitimate to use WIDTH*num_rows, since the process 0
                // always has the highest number of rows to calculate
                rc = MPI_Recv(color, WIDTH*num_rows, MPI_UNSIGNED_CHAR,
            ...
            for (int received_row = 0; received_row * WIDTH < received_length; ++received_row) {
                // copy one row to the correct offset in the image array
                assert(received_rank + received_row * size + WIDTH < WIDTH * HEIGHT);
                memcpy(image + received_rank + received_row * size * WIDTH, color + received_row * WIDTH, WIDTH * sizeof(unsigned char));
            }
        }
        ...
        if (output_mode == OUTPUTMODE_ASCII) {
            rc = write_ascii_image_to_disk(image, WIDTH, HEIGHT, NCOLORS, outfile_name);
        }
        #ifdef ENABLE_PNG
        else if (output_mode == OUTPUTMODE_PNG) {
            rc = write_png_image_to_disk(image, WIDTH, HEIGHT, NCOLORS, outfile_name, palette_name);
        }
        #endif
        ...
        rc = MPI_Send(color, WIDTH*num_rows, MPI_UNSIGNED_CHAR, 0, 0, MPI_COMM_WORLD);
        free(color);
    }
...
// this function takes an image array of length WIDTH*HEIGHT
// and produces a file in ascii format of the name <filename>.txt
int write_ascii_image_to_disk(unsigned char* image, const unsigned int WIDTH, const unsigned int HEIGHT,
    ...
    for (int i = HEIGHT; i > 0; --i) {
        for (int j = 0; j < WIDTH; ++j)
            fprintf(fp, "%hhu ", image[j + HEIGHT*(i-1)]);
...
int write_png_image_to_disk(unsigned char* image, const unsigned int WIDTH, const unsigned int HEIGHT,
    ...
    png_set_IHDR(png_write_ptr, png_info_ptr, WIDTH, HEIGHT, /*bit_depth*/ 8, PNG_COLOR_TYPE_PALETTE,
    ...
    for (int row_index = 0; row_index < HEIGHT; ++row_index) {
        // the index formula is derived from image + WIDTH*HEIGHT - row_index*WIDTH
        rows[row_index] = image + (HEIGHT - row_index) * WIDTH;
    }
